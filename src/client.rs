...
async fn client(
...
) -> anyhow::Result<impl Stream<Item = std::result::Result<Bytes, hyper::Error>>> {
    ...
    let stream = futures::stream::unfold(res, move |mut res| async move {
...
pub async fn echo_client() -> anyhow::Result<()> {
